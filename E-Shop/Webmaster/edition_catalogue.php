...
$c=mysql_connect("localhost","root","") or die("erreur de connexion");
		...
		$r=mysql_query("UPDATE catalogue SET libelle='$libelle',description='$description',image='$photo' WHERE refproduit=$id ");
...
$c=mysql_connect("localhost","root","") or die("erreur de connexion");
$base=mysql_select_db("projet_web") or die ("base non trouve");
$r=mysql_query("select * From catalogue Where refproduit=$id");
while($ligne=mysql_fetch_array($r)){
  echo " <h2>Modifier article : </h2>
   <div class=form>
         <form action=edition_catalogue.php?id=$ligne[refproduit] method=post class=niceform>
